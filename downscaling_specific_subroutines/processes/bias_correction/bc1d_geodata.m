...
for mm = 1 : numel(sSim2Bc)
    ...
    nRefHTs = numel(indRefH);
    nSimHTs = numel(indSimH);
    if nRefHTs ~= nSimHTs
        ...
        unqYrsSimH = unique(sSimH{mm}.(varDate)(indSimH,1));
        unqYrsRefH = unique(sRefH.(varDate)(indRefH,1));
        if numel(unqYrsSimH) ~= numel(unqYrsRefH)
            warning('eQMgeodata:unequalYrs', ['The number of years present in the '...
               'two datasets being compared are not equal (' ...
                num2str(numel(unqYrsSimH)) ' in historic simulation and ' ...
                num2str(numel(unqYrsRefH)) ' in historic reference). They are ' ...
